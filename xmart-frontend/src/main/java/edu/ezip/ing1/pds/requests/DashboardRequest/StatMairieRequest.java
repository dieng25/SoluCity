...
public class StatMairieRequest extends ClientRequest<DashboardFilterDTO, StatMairieDatas> {

    public StatMairieRequest(NetworkConfig networkConfig, int myBirthDate, Request request, DashboardFilterDTO filterDTO, byte[] bytes)
            throws IOException {
   ...
   private static byte[] convertToJson(DashboardFilterDTO filterDTO) throws IOException {
    ...
    @Override
    public StatMairieDatas readResult(String body) throws IOException {
        final ObjectMapper mapper = new ObjectMapper();
        return mapper.readValue(body, StatMairieDatas.class);
    }
}
